...
class exp2 extends JApplet 
{
	...
	class MyPanel extends JPanel  implements ActionListener , MouseMotionListener 
	{
		...
		var exp_type = 0 ; // 0 -> NAND , 1 -> NOR 
		var scale_x = 1 ;  // scaling of work pannel 
		var scale_y = 1 ;

		var work_x ;
		var work_y ;
		var wire_button  = 0 ;// 0 not presed already , 1 -> already pressed 
		var img_button_pressed = -1 ;  
		var draw_work = 0 ; // if 1 -> draw the image on work 
		...
		var work_img_width = 50;
		var work_img_height = 50;
		var work_panel_width = 720 ;
		var work_panel_height = 540;

		var node_drag = -1 ; // it rep the index of comp_node which is selected to be draged 
		var wire_drag = -1 ; // it rep the index of wire which is selected to be extented from its end 
		var wire_drag_end = 1 ; // from which end it should be draged 
		var comp_count = new Array(20); //  comp_count[i] represents the count of ( comp"i".jpg ) component .. [int type]
		var total_comp = 0 ;
		var comp_node = new Array(50); // [node type]

		var total_wire = 0 ;
		var wire = new Array(20); // [line type]
		...
		var comp_str = [		// This will store what should at the Dialog Box for each component
		...
		public 	class node 
		{
			var node_x ;
			var node_y ;
			var img_no ; // indicates which component 
			var comp_no ; // n -> (n)th comp ,  ie 1 -> first c comp ;; indicates which particular comp is this , if more than one  same type Components are present . 
			var width ;
			var height ;
			...
			var angle ;
			var angle_count ; // 0-> 0 / 360 degree , 1 -> 90 degree , 2 -> 180 degree , 3 -> 270 degree

			var del ;

			// for connection with wire 
			var end_pointsX = new Array(5);
			var end_pointsY = new Array(5);
			var count_end_points = 0 ;

			constructor ( x , y , no ,  w ,  h ,  compNo)
			{
				comp_no = compNo ;
				node_x = x ;
				node_y = y ;
			...
			function update_end_points_mat( img)
			{
				if ( img == 1) // pmos
				{
					if ( comp_no == 1 )
					{
						for ( k = 0 ; k < 3 ; k++ )
						{
							for ( i = end_pointsX[k] - 4 ; i < end_pointsX[k] + 5; i++ )
							{
								for ( j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j++ )
								{
				...
				else if ( img == 2 || img == 9 || img == 4 || img == 10 ) // ground , VDD , INPUT , OUTPUT
				{
					...
					for (  i = end_pointsX[0] - 4 ; i < end_pointsX[0] +5 ; i++ )
					{
						for ( j = end_pointsY[0] - 4 ; j < end_pointsY[0] +5; j++ )
						{
							if ( img == 2 ) // ground
							{
			...
			function make_end_points(img)
			{
				...
				this.update_end_points_mat(img);
			}
			function del ()
			{
				del = true ;
		...
		//		angle = angle +  (java.lang.Math.PI/2);
				...
				this.update_mat(index); // update the matrix value to work_mat // index is the index of the node_comp matrix
			}
			function update_mat(index) // update the matrix value to work_mat // index is the index of the node_comp matrix
			{
				var i , j ;
				for ( i = node_x ;  ;)
				{
				...
				this.make_end_points(img_no);
			}
			function remove_mat() // delete the previous value from work_mat
			{
				var i , j ;
					for ( i = node_x ;  ;  )
					{
			...
			function remove_end_points()
			{
				for ( k = 0 ; k < count_end_points ; k++ )
				{
					for ( i = end_pointsX[k] - 4 ; i < end_pointsX[k] +5 ; i++ )
					{
						for ( j = end_pointsY[k] - 4 ; j < end_pointsY[k] +5; j++ )
		...
		class line 
		{
			var x1 , y1 , x2 , y2 ; // end and start povar of wire 
		    var x = new Array(20);
			var y = new Array(20);
			var end_index ;
			var del ;
			...
			function update2(  c , d ) // update end point
			{
				x2 = c ;
				y2 = d ;
//				end_index++;
			...
			function update1( c ,  d ) // update start povar 
			{
				x[0] = x1 = c ;
				y[0] = y1 = d ;
			}
			function update( a , b ) // update middle povar 
			{
				end_index++;
				x[end_index] = a;
				y[end_index] = b ;
			}
			function update_wire_mat( index )
			{
				var i , j , tx1 , ty1 , tx2 , ty2 ; // local varibles 
		...
		public class myDialog extends JDialog implements ActionListener 
		{
			...
			JButton rotate ;
			var node_index ;
			constructor (JFrame fr , String comp, var node_no)
			{
				...
				{
					//Syntax: SpinnerNumberModel(initial_value, min, max, step)
					SpinnerModel capacitance_model = new SpinnerNumberModel(100, 0, 1000, 1);
					JLabel comp_name = new JLabel("<html><font size=4><b>"+comp+" "+comp_node[node_no].comp_no+"</b></font></html>" );
				...
				{	//Syntax: SpinnerNumberModel(initial_value, min, max, step)
					SpinnerModel length_model =	new SpinnerNumberModel(50, 0, 1000, 1);
					SpinnerModel width_model =	new SpinnerNumberModel(100, 0, 1000, 1);
			...
			function actionPerformed(ActionEvent e )
			{
				...
				{
					var i , j ;
					var t1 = comp_node[node_index].height ;
...
//					System.out.println(comp_node[node_index].angle);
			...
			{
				function windowClosing( WindowEvent ev)
				{
					setVisible(false);
				}
			}
		...
		public  class WorkPanel extends JPanel  implements  MouseMotionListener , MouseListener   
		{
			...
			{ 
				...
				//	System.out.println(total_wire);
					var x = (work_x % 10)>5 ? (work_x/20)*20+20 : (work_x/20)*20; // for making good 
					var y = (work_y % 10)>5 ? (work_y/20)*20+20 : (work_y/20)*20; // accurate wire povar around end points 
...
//					wire[total_wire-1 ].update2((work_x/20)*20 , (work_y/20)*20);
			...
			/*		var t1 = comp_node[total_comp-1].height ;
			...
			function mouseClicked(MouseEvent me) 
			{
				...
				if ( img_button_pressed == -1 ) // for selecting anything on work panel
				{
					if ( wire_mat[work_x][work_y] != -1 )
					{
						...
						var n = JOptionPane.showConfirmDialog( wire_f, "Do u want to Delete Wire ?","Wire", JOptionPane.YES_NO_OPTION);
						if ( n == 0 )
						{
					...
					{
						var temp = work_mat[work_x][work_y] ; 
						var temp1 = comp_node[temp].img_no ; // temp is no img no 
						...
						else if ( /*temp1 == 4 ||*/ temp1 == 10 )
						{
							...
							var n = JOptionPane.showConfirmDialog( in_output_f, "Do u want to Delete this comp ?","in_out", JOptionPane.YES_NO_OPTION);
							if ( n == 0 )
							{
								//		System.out.println("Deletded ");
								comp_node[temp].del() ;
				...
				/*	if ( end_points_mat[work_x][work_y] != - 1 ) // if end points r there 
					{
					var x = (work_x % 10)>5 ? (work_x/20)*20+20 : (work_x/20)*20;
					var y = (work_y % 10)>5 ? (work_y/20)*20+20 : (work_y/20)*20;
					if ( wire_button == 0 ) // button is pressed first time 
					{
						//	wire[total_wire++] = new line((work_x /20)*20, (work_y/20)*20 , (work_x/20)*20 , (work_y/20)*20);
					wire[total_wire++] = new line(x,y , x, y);
...
*/
			...
			/*		var t1 = comp_node[total_comp].height ;
			...
			*/		//--------------------------------------------------------------------
			...
			function mouseReleased(MouseEvent me) 
			{
				var i , j ;
				if ( wire_drag != -1 )
				{
			...
			function mouseEntered(MouseEvent me) 
			{
			}
			function mouseExited(MouseEvent me) 
			{
			}

			function mousePressed(MouseEvent me) 
			{
				var i , j ;
				work_x = me.getX();
				work_y = me.getY();
		...
		//For Images --------------------------------
			...
			void draw_nmos(Graphics2D g , var x , var y , var width , double angle )
			{
...
//				g.drawRect( x , y , 2*width , 4*width);
			...
			void draw_cmos(Graphics2D g , var x , var y , var width , double angle)
			{
...
//				g.drawRect( x , y , 2*width , 4*width);
			...
			void draw_capacitor(Graphics2D g , var x , var y , var width , double angle)
			{
				g.rotate(angle , x , y);
				g.setColor(Color.yellow);
//				g.drawRect( x , y , 3*width , 2*width);
			...
			void draw_ground(Graphics2D g , var x , var y , var width , double angle)
			{
				g.rotate(angle , x , y);
				g.setColor(Color.yellow);
//				g.drawRect( x , y , 2*width , 2*width);
			...
			void draw_vdd(Graphics2D g , var x , var y , var width , double angle)
			{
				g.rotate(angle , x , y);
				g.setColor(Color.yellow);
			...
			void draw_input(Graphics2D g , var x , var y , var width , double angle , var comp_no)
			{
				g.rotate(angle , x , y);	
				g.setColor(Color.yellow);
			...
			void draw_output(Graphics2D g , var x , var y , var width , double angle)
			{
		...
		public class graph extends JPanel
		{
			...
			function make_graph()
			{
				...
				function paint(Graphics g)
				{
					...
					System.out.println("Hi I am in the gpavar func of the exp1_graph class :)");
					var i , j ;
					Graphics2D g2d = (Graphics2D)g ;
					g2d.setStroke(new BasicStroke(2));
					// back ground 
...
//			bottom.add(h );
		...
		{
			var check_points_value[][] = new int[25][10] ; // each index will store corresponding  component for points of (comp povar no -> index )
			var i = 0 , j , j1 = 0, k1 = 0 , j2 = 0 , k2 = 0 , l = 0 ;
			for (  i = 0 ; i < 25 ; i ++ )
			{
				for (  j = 0 ; j < 10 ; j ++ )
				{
			...
			for ( var i = 0 ; i < work_panel_width ; i++ )
			{
				for ( var j = 0; j < work_panel_height ; j++ )
				{
			...
			var temp ;
			for (  i = 0 ; i < 19 ; i ++ ) // for making aal connected povar for each end_povar in its array ..
			{	
			...
			for (  i = 0 ; i < 19 ; i ++ )
			{
				...
				{
					return false ; // if any end povar of component is free then wrong circiut ...
				}
			}
		...
		function change_selected (var no)
		{
...
//				change_selected(5);
		...
		function mouseDragged(MouseEvent e) {}
